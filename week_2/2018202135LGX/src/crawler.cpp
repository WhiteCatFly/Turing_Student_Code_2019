...
void insert_URLs(std::string url, std::string source_code, std::set<std::string>& temp, std::queue<std::string>& tempqueue, std::set<std::string> &label){
    ...
    while(regex_search(iterStart, iterEnd, result, standard)){
        ...
        if(!temp.count(temp_result_copy) && filter_false(temp_result_copy) == false && filter_true(temp_result_copy) == true){
...
inline bool filter_true(const std::string result){
    for(int i = 0; i < sizeof(filter_true1) / SIZE_OF_STRING; i ++){
        if(result.find(filter_true1[i]) == -1) return false;
    }
    return true;
}

inline bool filter_false(const std::string result){
    for(int i = 0; i < sizeof(filter_false1) / SIZE_OF_STRING; i ++){
        if(result.find(filter_false1[i]) != -1) return true;
    }
    return false;
}
