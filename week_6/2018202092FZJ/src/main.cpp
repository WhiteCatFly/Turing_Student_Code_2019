...
string sensitive_words[100]={".css",".doc",".docx",".xls",".xlsx",".txt",".jpg",".png","bmp",
...
int main(int argc,char* argv[]){
    if(argc==2){//读入根目录 argv[1] is root_url
        root_url = argv[1];
        crawler run;
        run.Bfs();  
    }
    if(argc==4){//argv[1] is root_url
    root_url = argv[1];//argv[2] is "diary_to_be_crawled.txt"(里面包含了将要爬取的，和已经爬取了但是没有pop掉)
