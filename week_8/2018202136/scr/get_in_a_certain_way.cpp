#include "head.h"
void HtmlParser::get_in_a_certain_way(string content)
{
	...
	int location_first = 0, location_end = 0;
        ...
        else
        {
                while(location_first != string::npos)
                {
                        ...
                        if(a_way_already_be_HtmlParser.find(a_way_to_be_finded) == a_way_already_be_HtmlParser.end())
                        {
                                fout << a_way_to_be_finded << endl;
                        }
                        location_first = content.find(format_first,location_first+1);
                        a_way_to_be_finded.erase(0,len);
                }
        }
        return;
}
