#include "head.h"
void HtmlParser::getContent(string url)
{
        string command;
        string buffer;
        buffer = "HtmlParser.txt" ;
        command = "wget -O "+ buffer + " --tries=1 ";
        all[number].url_name = url;
        command = command + url;
        system(command.data());
        ifstream fin;
        ofstream fout;
	...
	string url_format2 ;
        ...
        else
        {
			...
			url_format2 = file[location_first+len1];
                        ...
                        {
                                if(url_already_be_HtmlParser.find(url_to_be_finded) == url_already_be_HtmlParser.end())
                                {
                                        fout << url_to_be_finded << endl;
                                        url_to_be_HtmlParser.push(url_to_be_finded);
                                }
                        }
