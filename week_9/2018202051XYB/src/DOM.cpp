...
void DOM::Node::showText(std::string &res)
{
    if(type == TYPE_TEXT)
        res.append(content);
}

void DOM::Node::printText(std::ostream &os)
{
    os << content << std::endl;
}
...
void DOM::extractTagAttr_(std::vector<std::string> &res, Node *node, std::string tagName, std::string attrName)
{
    if(node->tagName == tagName)
    {
    ...
    for(auto x : node->child)
        extractTagAttr_(res, x, tagName, attrName);
}

std::vector<std::string> DOM::extractTagAttr(std::string tagName, std::string attrName)
{
    std::vector<std::string> res;
    extractTagAttr_(res, root, tagName, attrName);
    return res;
}

void DOM::extractAllText_(std::string &res, Node *node)
{
    if(node->type == TYPE_TEXT)
        ...
        return;
    for(auto x : node->child)
        extractAllText_(res, x);
}
std::string DOM::extractAllText()
{
    std::string ret;
    extractAllText_(ret, root);
    return ret;
}

void DOM::printAllText_(std::ostream &os, Node *node)
{
    if(node->type == TYPE_TEXT)
        node->printText(os);
    else if(isSpecialTag(node->tagName))
